...
/**
 * Admin sign-in screen — mapped 1:1 from PatientLoginPage (same layout,
 * same illustration, same styling, same behavior). Only the identifier
 * field (email-only, per Admin login requirements) and the left-panel
 * heading copy have been changed for the Admin/Hospital Admin context.
 * Auth wiring is out of scope here — submission is simulated so the
 * full flow and states can be reviewed end-to-end.
 */
export default function AdminLoginPage() {
  ...
  if (!identifier.trim() || !password.trim()) {
    setError(
      "Enter your admin email address and password to continue."
    );
    return;
  }
    ...
    <div className="flex min-h-full flex-col overflow-y-auto lg:h-full lg:min-h-0 lg:flex-row lg:overflow-hidden">
      ...
      <div className="relative flex w-full shrink-0 flex-col overflow-hidden bg-gradient-to-br from-mx-green-soft via-mx-bg to-mx-blue-soft px-6 py-6 sm:px-10 sm:py-8 lg:h-full lg:w-1/2 lg:px-12 lg:py-8 xl:px-16">
        ...
        <motion.div
          ...
          className="relative z-10 shrink-0"
        >
          {/* CHANGED: Admin/Hospital Admin left-panel heading (was Patient's "Making Your Health Journey Simpler.") */}
          <h2 className="font-display text-2xl font-extrabold leading-[1.15] text-mx-ink sm:text-3xl">
            Empowering Your <br />
            <span className="text-[var(--mx-green-strong)]">Hospital Operations</span> <br />
            Seamlessly.
          </h2>
        </motion.div>
      ...
      <div className="flex w-full flex-1 flex-col justify-center px-6 py-6 sm:px-10 sm:py-8 lg:h-full lg:w-1/2 lg:overflow-y-auto lg:px-16 xl:px-20">
        ...
        >
          ...
          <h1 className="font-display mt-3 text-2xl font-bold text-mx-ink sm:text-[1.75rem]">Welcome Back</h1>
          <p className="mt-1 text-sm text-mx-ink-muted">Sign in to continue your health journey.</p>

          <form onSubmit={handleSubmit} noValidate className="mt-6 flex flex-col gap-4">
            <div className="rounded-mx-md transition-shadow duration-150 focus-within:shadow-[0_0_0_4px_var(--mx-green-soft)]">
              {/* CHANGED: Admin logs in with email only (was "ABDM ID or Email Address", type="text") */}
              <Input
            ...
            <div className="rounded-mx-md transition-shadow duration-150 focus-within:shadow-[0_0_0_4px_var(--mx-green-soft)]">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="patient-password" className="text-sm font-semibold text-mx-ink">
            ...
            <button
              ...
              onClick={() =>
                showToast({
                  ...
                  description: "This will be wired up alongside admin authentication.",
                })
              }

...
const LINKS: SettingsLink[] = [
  { label: "Profile", description: "Name, contact details, and emergency contact.", icon: UserRound, tone: "blue", path: "profile" },
...
export default function SettingsPage() {
  ...
  const [notifPrefs, setNotifPrefs] = useState({ lab: true, prescription: true, visit: true, system: false });
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <Card>
        ...
        <ul className="divide-y divide-mx-border">
          <PrefToggle
            label="Lab report updates"
            checked={notifPrefs.lab}
            onChange={() => toggleNotifPref("lab")}
          />
          <PrefToggle
            label="Prescription updates"
            checked={notifPrefs.prescription}
            onChange={() => toggleNotifPref("prescription")}
          />
          <PrefToggle
            label="Visit reminders"
            checked={notifPrefs.visit}
            onChange={() => toggleNotifPref("visit")}
          />
          <PrefToggle
